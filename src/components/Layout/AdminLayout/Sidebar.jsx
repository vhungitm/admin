          ...
          return (
            <div key={item.id} className={menuItemClassName} onClick={() => onClickMenuItem(item)}>
              <div className="sidebar-menu-item-content">
                <img src={'/media/layout/admin/' + item.icon} className="sidebar-menu-item-icon" alt={item.title} />
              ...
              {item.items && (
                <div className="sidebar-sub-menu">
                  ...
                  {item.items.map(subItem => {
                    const subItemIsActive = currPath.startsWith(subItem.href);
                    let subItemClassName = 'sidebar-sub-menu-item';
                    if (subItemIsActive) subItemClassName += ' is-active';

                    return (
                      <Link to={subItem.href} key={subItem.id} className={subItemClassName}>
                        {subItem.title}
                      </Link>
                    );
                  })}
